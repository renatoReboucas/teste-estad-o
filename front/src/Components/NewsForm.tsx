  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (isEdit && initialData?.id) {
        await newsApi.updateNews(initialData.id, submitFormData);
      } else {
        await newsApi.createNews(submitFormData);
      }
      onSuccess();
    } catch (error) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input
          ...
          className={`w-full p-2 border ${
            errors.editoria ? "border-red-500" : ""
          }`}
      ...
      <div>
        ...
        <input
          ...
          onChange={handleInputChange}
          className={`w-full p-2 border ${
            errors.subtitulo ? "border-red-500" : ""
          }`}
      ...
      <div>
        ...
        <Wysiwyg html={formData.conteudo} onChange={handleWysiwygChange} />
        {errors.conteudo && (
          <span className="text-red-500">Campo obrigatório</span>
        )}
      </div>
